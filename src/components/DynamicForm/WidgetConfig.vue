<template>
  <div class="form-config">
    <el-form label-position="top">
      ...
      <el-form-item v-if="data.type !== 'grid'" label="字段名称">
        <el-input v-model="data.name"/>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('width') >= 0" label="宽度">
        <el-input v-model="data.width"/>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('height') >= 0" label="高度">
        <el-input v-model="data.height"/>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('size') >= 0" label="大小">
        宽度
        <el-input v-model.number="data.size.width" style="width: 90px;" type="number"/>
        高度
        <el-input v-model.number="data.size.height" style="width: 90px;" type="number"/>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('inline')>=0" label="布局方式">
        <el-radio-group v-model="data.inline">
          <el-radio-button :label="false">块级</el-radio-button>
          <el-radio-button :label="true">行内</el-radio-button>
        </el-radio-group>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('showInput')>=0" label="显示输入框">
        <el-switch v-model="data.showInput"/>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('min')>=0" label="最小值">
        <el-input-number v-model="data.min" :min="0" :max="100" :step="1"/>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('max')>=0" label="最大值">
        <el-input-number v-model="data.max" :min="0" :max="100" :step="1"/>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('step')>=0" label="步长">
        <el-input-number v-model="data.step" :min="0" :max="100" :step="1"/>
      </el-form-item>
      <el-form-item v-if="data.type === 'select' || data.type === 'imgupload'" label="是否多选">
        <el-switch v-model="data.multiple" @change="handleSelectMultiple"/>
      </el-form-item>
      <el-form-item v-if="data.type === 'select'" label="是否可搜索">
        <el-switch v-model="data.filterable"/>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('allowHalf')>=0" label="允许半选)">
        <el-switch
          v-model="data.allowHalf"/>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('showAlpha')>=0" label="支持透明度选择">
        <el-switch
          v-model="data.showAlpha"/>
      </el-form-item>
      <el-form-item v-if="Object.keys(data).indexOf('remoteUrl') >= 0" label="远端数据">
        <el-input v-model="data.remoteUrl" placeholder="请输入访问地址"/>
      </el-form-item>
      <el-form-item
        v-if="Object.keys(data).indexOf('defaultValue')>=0 && (data.type === 'textarea' || data.type === 'input' || data.type === 'rate' || data.type === 'color' || data.type === 'switch')"
        label="默认值">
        <el-input
          ...
          :rows="5"
          type="textarea"/>
        <el-input v-if="data.type === 'input'" v-model="data.defaultValue"/>
        <el-rate
          v-if="data.type === 'rate'"
          v-model="data.defaultValue"
          :max="data.max"
        ...
        <el-switch v-if="data.type === 'switch'" v-model="data.defaultValue"/>
      </el-form-item>
      <template v-if="data.type === 'time' || data.type === 'date'">
        ...
        <el-form-item
          v-if="(!data.beRange && data.type === 'time') || (data.type !== 'time' && data.type !== 'datetimerange' && data.type !== 'daterange')"
          label="占位内容">
          <el-input v-model="data.placeholder"/>
        </el-form-item>
        <el-form-item
          v-if="(data.beRange) || data.type === 'datetimerange' || data.type === 'daterange'"
          label="开始时间占位内容">
        ...
        <el-form-item v-if="data.type ==='time' && Object.keys(data).indexOf('beRange')>=0" label="默认值">
          <el-time-picker
            v-if="!data.beRange"
            key="1"
            v-model="data.defaultValue"
      ...
      <template v-if="data.type === 'imgupload'">
        <el-form-item label="最大上传数">
          <el-input v-model.number="data.length" type="number"/>
        </el-form-item>
      ...
      <template v-if="data.type === 'grid'">
        ...
        <el-form-item label="列配置项">
          <el-col v-for="(item, index) in data.columns" :key="index">
            <el-input
        ...
        <el-form-item label="垂直排列方式">
          <el-select v-model="data.align" value="">
      ...
      <template v-if="data.type !== 'grid'">
        <el-form-item label="操作属性">
          <el-checkbox v-if="Object.keys(data).indexOf('readonly')>=0" v-model="data.readonly">完全只读
          </el-checkbox>
          <el-checkbox v-if="Object.keys(data).indexOf('disabled')>=0" v-model="data.disabled">禁用
          </el-checkbox>
          <el-checkbox v-if="Object.keys(data).indexOf('editable')>=0" v-model="data.editable">文本框可输入
          </el-checkbox>
          <el-checkbox v-if="Object.keys(data).indexOf('clearable')>=0" v-model="data.clearable">
            显示清除按钮
          </el-checkbox>
          <el-checkbox v-if="Object.keys(data).indexOf('arrowControl')>=0" v-model="data.arrowControl">
            使用箭头进行时间选择
          </el-checkbox>
          <el-checkbox v-if="Object.keys(data).indexOf('isDelete')>=0" v-model="data.isDelete">删除
          </el-checkbox>
          <el-checkbox v-if="Object.keys(data).indexOf('isEdit')>=0" v-model="data.isEdit">编辑
          </el-checkbox>
