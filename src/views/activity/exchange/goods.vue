<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.gcsId" placeholder="商品分类" clearable style="width: 120px" class="filter-item">
    ...
    <el-table
      ...
      <el-table-column label="创建时间" width="200" align="center">operationUser
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" width="340" class-name="small-padding fixed-width" fixed="right" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="warning" size="small" @click="handleClickRuleGoods(scope.row.id)">添加规则</el-button>
          <el-button type="primary" size="small" @click="handleRule(scope.row.id)">查看规则</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="dialogRuleVisible" title="添加换购商品规则" width="40%">
      <el-select v-model="selectedRuleId" clearable placeholder="请选择">
        <el-option v-for="item in ruleData" :key="item.id" :value="item.id" :label="item.ruleName" />
      </el-select>
    ...
    <el-dialog :visible.sync="dialogRuleInfoVisible" title="换购商品规则详情" width="40%">
      <el-collapse v-model="activeRule" accordion>
        <el-collapse-item title="活动规则详情" name="1">
          <div>活动名称：<span>{{ activeRuleForm.activityName }}</span></div>
          <div>规则名称：<span>{{ activeRuleForm.ruleName }}</span></div>
          <div>最低限额：<span class="icon-money"><svg-icon icon-class="money" />{{ activeRuleForm.orderAmount }}</span></div>
...
export default {
  ...
  methods: {
    ...
    handleRuleGoods() {
      const params = { activityId: this.listQuery.activityId, ruleId: this.selectedRuleId, goodsId: this.selectedGoodsId }
