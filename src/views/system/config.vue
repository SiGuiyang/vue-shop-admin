<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-permission="'/admin/system/config/modify'" class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate">{{ $t('table.add') }}</el-button>
    </div>
    ...
    <el-table
      ...
      <el-table-column :label="$t('table.actions')" class-name="small-padding fixed-width" fixed="right" align="center">
        <template slot-scope="scope">
          <el-button v-permission="'/admin/system/config/modify'" type="primary" size="mini" @click="handleUpdate(scope.row)">{{ $t('table.edit') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" width="50%">
        <el-form-item :label="$t('system.config.configName')" prop="configName">
          <el-input :disabled="configNameDisabled" v-model="temp.configName" placeholder="请设置"/>
        </el-form-item>
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.temp.operationUser = this.$store.state.user.sysCode
          this.temp.event = 'add'
